...
def update_monthly_sp500():
    print("🇺🇸 S&P 500 월말 데이터 업데이트 시작...")
    sp500 = fdr.StockListing('S&P500')
    os.makedirs('data', exist_ok=True)
    
    today = datetime.today()
    last_month_end = today.replace(day=1) - timedelta(days=1)
    start_date = last_month_end - pd.DateOffset(months=15)
    results = []
    
    for _, row in sp500.iterrows():
        sym, name = row['Symbol'], row['Name']
        try:
            df = fdr.DataReader(sym, start_date, last_month_end)
            ...
            results.append({
                '기준일(월말)': last_date.strftime('%Y-%m-%d'),
                '시장': 'NYSE',
    ...
    res_df.to_csv('data/momentum_data_sp500.csv', index=False, encoding='utf-8-sig') # 파일명 다름!
    print("✅ 월말 완료: data/momentum_data_sp500.csv")

if __name__ == "__main__":
    update_monthly_sp500()
