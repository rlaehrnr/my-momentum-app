...
# ⭐ 탭 3: KOSPI 200 집중 분석 (우선주 제외 필터 포함) ⭐
with tab3:
    if os.path.exists(f_kr):
        df_k200 = pd.read_csv(f_kr, dtype={'종목코드': str})
        b_date_str = df_k200['기준일(월말)'].iloc[0]
        st.title(f"🎯 KOSPI 200 강세 종목 (기준: {b_date_str})")
        ...
        # ⭐ 1. 우선주 필터링 및 KOSPI 추출 ⭐
        # 보통주는 종목코드 끝자리가 '0'으로 끝남. ('우', '우B' 등이 붙은 종목코드 필터링)
        # 만약 데이터프레임의 종목코드(str) 끝자리가 '0'이 아니면 우선주로 간주하여 제외
        is_common_stock = df_k200['종목코드'].str.endswith('0')
        is_kospi = df_k200['시장'] == 'KOSPI'
        
        # 코스피 + 보통주(우선주 제외) 조건으로 필터링한 후 상위 200개 자르기
        df_k200 = df_k200[is_kospi & is_common_stock].head(200).copy()
        
        df_k200['통합티커'] = df_k200['시장'] + ":" + df_k200['종목코드'].str.zfill(6)
        df_k200['종목명'] = df_k200.apply(lambda r: f"https://m.stock.naver.com/fchart/domestic/stock/{r['종목코드'].zfill(6)}#{r['종목명']}", axis=1)

        k200_col_order = ['통합티커', '종목명', '기준가', '1개월(%)', '3개월(%)', '6개월(%)', '12개월(%)']
        k200_col_config = {
            "종목명": st.column_config.LinkColumn("종목명", display_text=r"#(.+)"), 
            "기준가": st.column_config.NumberColumn("현재가", format="%,d"), 
            "1개월(%)": st.column_config.NumberColumn(format="%.1f"), 
        ...
        # --- [왼쪽 박스: 퍼펙트 상승 종목 (우선주 없음)] ---
        with col1:
            st.subheader("🔥 퍼펙트 상승 (All 양수 & 상위 30%)")
            ...
            top_30_1m = df_k200['1개월(%)'].quantile(0.70)
            top_30_3m = df_k200['3개월(%)'].quantile(0.70)
            top_30_6m = df_k200['6개월(%)'].quantile(0.70)
            top_30_12m = df_k200['12개월(%)'].quantile(0.70)
            
            cond_all_top30 = (
                (df_k200['1개월(%)'] >= top_30_1m) & 
                (df_k200['3개월(%)'] >= top_30_3m) & 
                (df_k200['6개월(%)'] >= top_30_6m) & 
                (df_k200['12개월(%)'] >= top_30_12m)
            )
            
            df_perfect = df_k200[cond_all_positive & cond_all_top30].copy()
            df_perfect = df_perfect.sort_values(by='12개월(%)', ascending=False).reset_index(drop=True)
            df_perfect.index = range(1, len(df_perfect) + 1)
            
            if not df_perfect.empty:
                st.dataframe(df_perfect.style.apply(apply_custom_styling, idx_df=idx_now_k200, axis=1), 
                             use_container_width=True, height=500, 
                             column_order=k200_col_order, column_config=k200_col_config)
            else:
                st.info("조건에 맞는 종목이 없습니다.")

        # --- [오른쪽 박스: 장기 주도 & 단기 급등 (우선주 없음)] ---
        with col2:
            st.subheader("🚀 장기 주도 & 단기 급등 (12M 30% & 1M 10%)")
            
            top_10_1m = df_k200['1개월(%)'].quantile(0.90)
            
            cond_special = (
                (df_k200['12개월(%)'] >= top_30_12m) & 
                (df_k200['1개월(%)'] >= top_10_1m)
            )
            
            df_special = df_k200[cond_special].copy()
            df_special = df_special.sort_values(by='12개월(%)', ascending=False).reset_index(drop=True)
            df_special.index = range(1, len(df_special) + 1)
            
            if not df_special.empty:
                st.dataframe(df_special.style.apply(apply_custom_styling, idx_df=idx_now_k200, axis=1), 
                             use_container_width=True, height=500, 
                             column_order=k200_col_order, column_config=k200_col_config)
            else:
                st.info("조건에 맞는 종목이 없습니다.")
