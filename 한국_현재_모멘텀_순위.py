...
# 🎨 하이라이트 함수 (고대비: 짙은 파랑 배경 + 흰색 굵은 글씨)
...
with tab1:
    ...
    if os.path.exists(f_kr):
        ...
        # ⭐ 종목 표 출력 (column_config로 소수점 1자리 강제 고정)
        st.dataframe(
            ...
            column_config={
                ...
                "모멘텀스코어": st.column_config.NumberColumn(format="%.1f"), # ⭐ 스코어도 1자리로 통일
            }
        )
    else: st.warning("데이터 파일이 없습니다.")
...
with tab2:
    ...
    if os.path.exists(f_daily):
        ...
        # ⭐ 데일리 종목 표 출력 (column_config로 소수점 1자리 강제 고정)
        st.dataframe(
    ...
    else: st.warning("데일리 데이터 파일이 아직 생성되지 않았습니다.")
