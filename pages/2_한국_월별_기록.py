...
if not files:
    ...
    selected_file = file_map[st.selectbox("조회할 달 선택", list(file_map.keys()))]

    df = pd.read_csv(selected_file, dtype={'종목코드': str})
    
    # ⭐ 시장 필터 추가
    mkts = st.multiselect("🔎 시장 필터", options=df['시장'].unique(), default=df['시장'].unique())
    df = df[df['시장'].isin(mkts)].copy()

    st.success(f"✅ 기준일: {df['기준일(월말)'].iloc[0]}")
    ...
    win_rate = (df['다음달수익률(%)'] > 0).sum() / len(df) * 100 if not df.empty else 0
    c1, c2 = st.columns(2)
    c1.metric("평균 수익률 (필터기준)", f"{avg_ret:.1f}%")
    c2.metric("상승 종목 비율 (필터기준)", f"{win_rate:.1f}%")
