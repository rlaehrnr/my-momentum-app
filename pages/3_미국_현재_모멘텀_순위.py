...
# ⭐ 교집합 중복 하이라이트 함수 (양쪽 교집합 표에 모두 있는 종목 강조)
def highlight_double_overlap(row, common_tickers):
    styles = [''] * len(row)
    if row.get('종목코드') in common_tickers:
        # 트리플 크라운 종목은 황금색으로 강조
        for i in range(len(styles)):
            styles[i] = 'background-color: #FFF9C4; color: #856404; font-weight: bold;'
    return styles
...
def display_momentum_dashboard(df_raw, target_date_str):
    ...
    # 두 교집합 표에 모두 등장하는 종목 찾기 (강조용)
    common_between_overlaps = set(overlap_12_6['종목코드']).intersection(set(overlap_6_3['종목코드']))
    ...
    with c_over1:
        ...
        if not overlap_12_6.empty:
            # 💡 정렬과 상관없이 고정된 번호(#) 부여 (index+1 활용)
            overlap_12_6 = overlap_12_6.reset_index(drop=True)
            overlap_12_6.index += 1
            st.dataframe(overlap_12_6.style.apply(highlight_double_overlap, common_tickers=common_between_overlaps, axis=1), 
    ...
    with c_over2:
        ...
        if not overlap_6_3.empty:
            ...
            overlap_6_3.index += 1
            st.dataframe(overlap_6_3.style.apply(highlight_double_overlap, common_tickers=common_between_overlaps, axis=1), 
                         use_container_width=True,
    ...
    for col, title, sort_col in zip([col1, col2, col3], 
        ...
        with col:
            ...
            df_sub.index += 1 # 💡 고정 번호 부여
            st.dataframe(df_sub, use_container_width=True, height=450,
    ...
    df_300_all.index += 1 # 💡 시총 순위 고정 번호
    st.dataframe(df_300_all, use_container_width=True, height=600,
                 column_order=['통합티커', '종목명_L', '기준가', '3-1개월(%)', '6-1개월(%)', '12-1개월(%)'],
                 column_config=common_config)

# 실행 로직
st.title("🇺🇸 미국 시총상위 모멘텀")
t1, t2, t3 = st.tabs(["🎯 전체 순위", "📅 전월 말일", "🕒 데일리"])
