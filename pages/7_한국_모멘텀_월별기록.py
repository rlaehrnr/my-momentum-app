...
# 💡 강제 줄바꿈 및 어긋남을 유발하던 여백/마진 CSS를 모두 제거하고 순정 상태로 최적화했습니다.
st.markdown("""
    <style>
...
with tab_detail:
    ...
    # 💡 억지 여백 코드를 모두 지우고 순정 st.columns 배치로 완벽한 수평 정렬 구현
    ...
    cond_perf_tab1 = (df_monthly['1개월(%)']>=q30['1개월(%)']) & (df_monthly['3개월(%)']>=q30['3개월(%)']) & \
                     (df_monthly['6개월(%)']>=q30['6개월(%)']) & (df_monthly['12개월(%)']>=q30['12개월(%)']) & \
                     (df_monthly['1개월(%)']>0) & (df_monthly['3개월(%)']>0) & (df_monthly['6개월(%)']>0) & (df_monthly['12개월(%)']>0)
                     
    # 💡 달리는 말: 0 이상 수익률 강제 조건 완벽 삭제
    cond_spec_tab1 = (df_monthly['12개월(%)']>=q30['12개월(%)']) & (df_monthly['1개월(%)']>=t10_1m)

    df_perf = df_monthly[cond_perf_tab1].sort_values('3개월(%)', ascending=False).copy()
    df_spec = df_monthly[cond_spec_tab1].sort_values('1개월(%)', ascending=False).copy()
    ...
    with c_left:
        ...
        st.markdown('<p class="strategy-desc">전체 시장 중 1, 3, 6, 12개월 수익률이 모두 상위 30% 이내이며 0보다 큰 종목 (3개월 수익률 순)</p>', unsafe_allow_html=True)
    ...
    with c_right:
        ...
        st.markdown('<p class="strategy-desc">전체 시장 중 12개월 수익률 상위 30% 이내, 1개월 수익률 상위 10% 이내인 종목 (1개월 수익률 순)</p>', unsafe_allow_html=True)
...
with tab_summary:
    ...
    with st.spinner("수익률 계산 중..."):
        ...
        for m in months:
            ...
            cond_p = (m_data['1개월(%)']>=q_val_1)&(m_data['3개월(%)']>=q_val_3)&(m_data['6개월(%)']>=q_val_6)&(m_data['12개월(%)']>=q_val_12) & \
            ...
            ret_combined = (df_combined['다음달수익률(%)'].mean() * mult) if not df_combined.empty else 0.0
            
            records.append({
                '투자월': inv_str, 
                'invested': (mult > 0), 
                f'🔥 퍼펙트 상승 ({rank_1_start}~{rank_1_end}위)': ret_1, 
                f'🐎 달리는 말 ({rank_2_start}~{rank_2_end}위)': ret_2, 
                '앙상블 (전략 50:50)': (ret_1 + ret_2) / 2,
                '통합 (모든종목 동일비중)': ret_combined
            })
            
            if mult == 0.0:
                trade_logs_tab2.append({'투자월': inv_str, '전략': '마켓타이밍 작동', '매수순위': '-', '종목명': '현금 (투자중지)', '종목코드': '-', '수익률(%)': 0.0})
            else:
                for i, (_, row) in enumerate(overlap_1.iterrows()):
                    trade_logs_tab2.append({'투자월': inv_str, '전략': '🔥 퍼펙트 상승', '매수순위': f"{i + rank_1_start}위", '종목명': row['종목명'], '종목코드': row['종목코드'], '수익률(%)': row['다음달수익률(%)']})
                for i, (_, row) in enumerate(overlap_2.iterrows()):
                    trade_logs_tab2.append({'투자월': inv_str, '전략': '🐎 달리는 말', '매수순위': f"{i + rank_2_start}위", '종목명': row['종목명'], '종목코드': row['종목코드'], '수익률(%)': row['다음달수익률(%)']})
        ...
        if not df_res.empty:
            ...
            st.download_button(
                ...
                data=csv_data_t2,
                file_name=f"전체시장_다중조건_백테스트_{datetime.today().strftime('%Y%m%d')}.csv",
                mime="text/csv",
...
with tab_custom:
    ...
    with c8: rank_c_start, rank_c_end = st.slider("🏅 매수 순위 범위 ", 1, 50, (1, 10))
    ...
    with st.spinner("계산 중..."):
        ...
        for m in months_c:
            ...
            inv_str = (pd.to_datetime(m) + pd.DateOffset(months=1)).strftime('%Y-%m')
            ...
            q_val_c = m_data['CustomScore'].quantile(q_c)
            
            target_group = m_data[(m_data['CustomScore']>=q_val_c)].sort_values('CustomScore', ascending=False).iloc[rank_c_start-1 : rank_c_end]
            ...
            if mult == 0.0:
                trade_logs_tab3.append({'투자월': inv_str, '전략': '마켓타이밍 작동', '매수순위': '-', '종목명': '현금 (투자중지)', '종목코드': '-', '수익률(%)': 0.0})
            else:
                for i, (_, row) in enumerate(target_group.iterrows()):
                    trade_logs_tab3.append({'투자월': inv_str, '전략': '커스텀 스코어', '매수순위': f"{i + rank_c_start}위", '종목명': row['종목명'], '종목코드': row['종목코드'], '수익률(%)': row['다음달수익률(%)']})
        ...
        if not df_res_c.empty:
            ...
            st.download_button(
                ...
                data=csv_data_t3,
                file_name=f"전체시장_커스텀_백테스트_{datetime.today().strftime('%Y%m%d')}.csv",
                mime="text/csv",
                use_container_width=True
            )
