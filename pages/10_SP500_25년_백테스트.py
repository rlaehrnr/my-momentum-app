...
def get_market_timing():
    ...
    # 월말 데이터만 추출
    timing_df = sp500.resample('M').last()
...
with st.spinner("25년 치 12만 개 이상의 데이터를 시뮬레이션 중입니다... 🚀"):
    ...
    for m in months:
        ...
        # 1. 12개월 & 6개월 모멘텀 교집합 (Top N)
        # 상위 3배수 추출 후 교집합을 구합니다.
...
fig = px.line(df_melt, x='YearMonth', y='누적수익률', color='전략', log_y=True) # 💡 핵심: 25년 장기투자용 로그 스케일 적용!
fig.update_layout(
    hovermode="x unified",
    dragmode="pan", 
    xaxis_title="투자 기준 월",
