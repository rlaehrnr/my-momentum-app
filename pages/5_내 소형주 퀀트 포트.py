...
st.markdown("""
    ...
    /* 💡 다크모드에 찰떡같이 어울리는 요약 표 디자인 */
    .summary-table { width: 100%; border-collapse: collapse; text-align: center; font-size: 1.2rem; background-color: #111827; border-radius: 8px; overflow: hidden; }
    ...
    .summary-total { background-color: #1F2937; font-size: 1.4rem; }
...
# --- [2. 설정 파일 로드/저장 (개별 수익금)] ---
def load_config():
    default_config = {"start_ddo": 0, "start_sso": 0, "start_mom": 0}
...
with tab_summary:
    ...
    if new_ddo != config['start_ddo'] or new_sso != config['start_sso'] or new_mom != config['start_mom']:
    ...
    for p_name, p_key, path in ports:
        ...
        if not df.empty:
            ...
            summary_data.append({
                ...
                "오늘의 등락": d_diff,
                "총 수익률": t_pct,
                "현재 수익 금액": final_port_profit
            })
        else:
            summary_data.append({"포트폴리오": p_name, "오늘의 등락": 0, "총 수익률": 0.0, "현재 수익 금액": start_val})

    final_profit = total_profit_all + config['start_ddo'] + config['start_sso'] + config['start_mom']
