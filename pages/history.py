...
# ⭐ 사람인 척 위장(User-Agent)해서 GitHub에서 데이터를 가져오는 함수
...
def get_marcap_data(year):
    ...
    response.raise_for_status() # 연결 실패 시 여기서 바로 에러를 띄움
    
    # 다운받은 압축파일(gz) 데이터를 판다스로 읽기
    df = pd.read_csv(io.BytesIO(response.content), compression='gzip', dtype={'Code': str}, parse_dates=['Date'])
    return df
...
if st.button(f"🚀 {selected_year}년 {selected_month}월 기준 상위 30위 추출하기"):
    ...
    if df_history is not None and not df_history.empty:
        ...
        st.warning("데이터를 불러오는 중 문제가 발생했습니다. (빨간색 에러 메시지를 확인해 주세요)")
