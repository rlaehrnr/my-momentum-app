...
with col1:
    selected_year = st.selectbox("연도 선택", range(2015, 2027), index=9) # 2024년을 기본값으로
with col2:
...
if st.button(f"🚀 {selected_year}년 {selected_month}월 기준 상위 30위 추출하기"):
    ...
    if df_history is not None and not df_history.empty:
        ...
        # ⭐ 1. 통합티커 컬럼 추가 (시장 + 종목코드)
        df_history['통합티커'] = df_history['시장'] + ":" + df_history['종목코드']
        
        # ⭐ 2. 보여줄 컬럼 순서 지정 (숨긴 컬럼 복구 및 맨 앞에 통합티커 배치)
