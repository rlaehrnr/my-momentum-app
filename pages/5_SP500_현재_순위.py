...
# ⭐ 구글 파이낸스 일봉 차트 링크 생성 함수 (0초 리다이렉트!)
def get_google_finance_link(row):
    symbol = str(row['종목코드']).strip().upper()
    market = str(row['시장']).strip().upper()
    # 구글 파이낸스 형식: https://www.google.com/finance/quote/TICKER:MARKET
    return f"https://www.google.com/finance/quote/{symbol}:{market}#{row['종목명']}"
...
with tab1:
    ...
    if os.path.exists(f_monthly):
        ...
        # ⭐ 구글 파이낸스 링크 적용
        df_m['종목명_L'] = df_m.apply(get_google_finance_link, axis=1)
...
with tab2:
    ...
    if os.path.exists(f_daily):
        ...
        # ⭐ 구글 파이낸스 링크 적용
        df_d['종목명_L'] = df_d.apply(get_google_finance_link, axis=1)
