...
def run_daily(market_type='KR'):
    if market_type == 'KR':
        ...
        # ⭐ S&P 500 종목의 실제 거래소 구분을 위한 매핑 데이터 준비
    ...
    for mkt_name in market_list:
        ...
        for _, row in target_stocks.iterrows():
            try:
                ...
                # 시장 이름 결정 (SP500은 NYSE/NASDAQ 상세 구분)
                display_mkt = mkt_map.get(code, 'NYSE') if market_type == 'SP500' else mkt_name

                res.append({
                    '기준일': today.strftime('%Y-%m-%d'), '시장': display_mkt,
                    '종목명': row['Name'], '종목코드': code, '기준가': round(curr_price, 2),
